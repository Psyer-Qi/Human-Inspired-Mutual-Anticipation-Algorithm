function [path,cost_all,flag] = runAAlgorithm(info)
%runAAlgorithm Detour static avatars
flag = true;
while ~isempty(info.Open)
    ...
    for i = 1:size(neighbors,1)
        ...
        for j = 1:size(info.Blocker,2)
            cost_space_human = cost_space_human + info.w_human_space * getPersonalSpaceCost(info,info.Blocker{j},info.ori{j},info.Blocker_v{j},sn); % Human Personal Space
