...
def distance(pose1, pose2):
    ...
    return sqrt((pose1[0] - pose2[0]) ** 2 + (pose1[1] - pose2[1]) ** 2) + 0.002
    # return sqrt(1/4 * (pose1[0]-pose2[0])**2+(pose1[1]-pose2[1])**2)+0.001
...
def compute_v_des(X, goal, V_max):
    ...
    for i in range(len(X)):
        ...
        norm = distance(dif_x, [0, 0])  # it's L2 norm
        norm_dif_x = [dif_x[k] * V_max[i] / norm for k in range(2)]
        # norm_dif_x = [dif_x[k]*V_max[k]/norm for k in range(1)] # to debug
...
# 0.3m implies that the robot has reached the destination
def reach(p1, p2, bound=0.01):
